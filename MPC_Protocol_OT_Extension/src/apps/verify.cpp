...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // Replay queries directly with oblivious transfer for index confidentiality
        auto queries = read_queries("queries.txt");
        for (const auto& q : queries) {
            int ui = q.first;
            int vj = oblivious_transfer(q.second); // Use oblivious transfer to get vj securely
